...
class ChatCubit extends Cubit<List<Message>> {
  ...
  void chatOpenEvent() {
    emit([...state, Message.buddyTalk(imageUrl: buddy.imageUrl)]);
  ...
  void sendMessage(String message, {String? event}) {
    ...
    final under18 = client.adult ? "" : "User is a minor under 18 years of age;";
    final eventDescription =
        "$under18 User sending this message is called ${client.name}; $under18";
  ...
  Future<void> fetchMessages() async {
    try {
      ...
      if (rowList.isNotEmpty && rowList.last['role'] == 'user') {
        messages.add(Message.buddyTalk(imageUrl: buddy.imageUrl));
      }

      if (messages.isNotEmpty) {
        messages.insert(1, Message.info(text: "All responses are generated by Gemini AI"));
      }
...
class Message {
  ...
  factory Message.standard(
      {required String text, required bool isBuddy, required String imageUrl}) {
    return Message(text: text, isBuddy: isBuddy, imageUrl: imageUrl, infoMessage: false);
  }

  factory Message.info({required String text}) {
    return Message(
      text: text,
      isBuddy: true,
      imageUrl: '',
  ...
  factory Message.buddyTalk({required String imageUrl}) {
    return Message(
      text: "🔵🔵🔵",
      isBuddy: true,
      imageUrl: imageUrl,
